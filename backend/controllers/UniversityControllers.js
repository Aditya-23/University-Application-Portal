...
import { getAllUniversitiesService, saveUniversityService } from "../services/UniversityServices.js";
...
export const registerUniversity = async (req, res) => {
    try {
        ...
        if(!savedUniversityObj){
            return setRequestError({msg: "Could not register the university"}, res);
        }
...
export const getAllUniversities = async (req, res) => {
    try {
        ...
        if(allUniversities.length == 0){
            return setRequestError({msg: "No university found"}, res);
        }
